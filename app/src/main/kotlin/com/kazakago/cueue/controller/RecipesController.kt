...
class RecipesController(private val userRepository: UserRepository, private val recipeRepository: RecipeRepository) {
    ...
    suspend fun index(call: ApplicationCall, firebaseUser: FirebaseUser, workspaceId: WorkspaceId, afterId: RecipeId?, tagId: TagId?) {
    ...
    suspend fun create(call: ApplicationCall, firebaseUser: FirebaseUser, workspaceId: WorkspaceId, recipe: RecipeRegistrationData) {
        val user = userRepository.getUser(firebaseUser.uid)
        user.validate(workspaceId)
